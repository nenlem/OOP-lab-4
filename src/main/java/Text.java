 ...
 *
 * Текст розбивається на речення за розділовими знаками кінця речення ({@code .}, {@code !},
 * {@code ?}), при цьому ці роздільники залишаються приєднаними до попереднього речення.
 * Пробіл між реченнями зберігається як початковий пробіл у наступному токені речення.
 */
public class Text {
